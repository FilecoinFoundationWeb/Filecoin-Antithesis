...
echo "Waiting for drand dkg to be executed"
drand_1_ready=0
...
cp /lotus/config-node1.toml "${LOTUS_DATA_DIR}/config-node1.toml"
cat localnet.json | jq -r '.NetworkName' > ${LOTUS_DATA_DIR}/network_name
cp localnet.json ${LOTUS_DATA_DIR}/localnet.json
lotus daemon --lotus-make-genesis=${LOTUS_DATA_DIR}/devgen.car --genesis-template=localnet.json --bootstrap=false --config=${LOTUS_DATA_DIR}/config-node1.toml&
lotus wait-api
...
touch /container_ready/lotus-1-node
