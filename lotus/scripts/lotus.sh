...
if [ ! -f "${LOTUS_DATA_DIR}/key" ] || [ ! -f "${LOTUS_DATA_DIR}/config.toml" ] || [ ! -f "${LOTUS_DATA_DIR}/devgen.car" ]; then
    INIT_MODE=true
...
if [ "$INIT_MODE" = "true" ]; then
    ...
    while [ "$tries" -gt 0 ]; do
        ...
        sleep 3
        tries=$(( tries - 1 ))
        echo "lotus${no}: $tries connection attempts remaining..."
    done
fi
    ...
    for peer in "${LOTUS_2_DATA_DIR}/ipv4addr" "${FOREST_DATA_DIR}/forest-listen-addr"; do
        if [ -f "$peer" ]; then
            ...
            while [ $attempt -le $max_attempts ]; do
                echo "lotus${no}: Attempting to connect to peer from $peer (attempt $attempt/$max_attempts)"
...
if [ "$INIT_MODE" = "true" ]; then
    ...
    # NOT DYNAMIC. How to specify a different miner based on the lotus node no?
    cp ${SHARED_CONFIGS}/.genesis-sector-1/pre-seal-t01000.key ${LOTUS_DATA_DIR}/key
    ...
    # Start daemon with genesis creation -- does the config.toml even exist here?
    lotus daemon --lotus-make-genesis=${LOTUS_DATA_DIR}/devgen.car --genesis-template=${SHARED_CONFIGS}/localnet.json --bootstrap=false --config=${LOTUS_DATA_DIR}/config.toml&
else
    echo "lotus${no}: Running in daemon-only mode..."
    # Start daemon without genesis creation -- similar to above comment, does the config.toml even exist here?
    lotus daemon --bootstrap=false --config=${LOTUS_DATA_DIR}/config.toml&
fi
