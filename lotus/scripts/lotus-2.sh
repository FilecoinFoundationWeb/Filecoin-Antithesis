...
if [ "$INIT_MODE" = "true" ]; then
    ...
    cat > "${LOTUS_2_DATA_DIR}/config.toml" <<EOF
[API]
  ListenAddress = "/ip4/${LOTUS_2_IP}/tcp/${LOTUS_2_RPC_PORT}/http"
EOF
    
    cat ${SHARED_CONFIGS}/localnet-2.json | jq -r '.NetworkName' > ${LOTUS_2_DATA_DIR}/network_name
