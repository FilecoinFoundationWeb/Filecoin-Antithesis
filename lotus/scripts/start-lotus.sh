...
# Always regenerate config — Antithesis may assign a new IP after container restart
sed "s|\${LOTUS_RPC_PORT}|$LOTUS_RPC_PORT|g; s|\${hostname}|lotus${node_number}|g" config.toml.template > config.toml
