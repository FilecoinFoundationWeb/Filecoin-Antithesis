...
//   FIP0115_PRE_LEAD_EPOCHS (default 2)   — flood starts this many epochs
//                                            before upgrade
//
...
// the deck for ~DURATION_SEC because `nonces` has no mutex and deck
// actions are otherwise serial.
// ===========================================================================

func init() {
	RegisterUpgradeVector(doFIP0115BaseFeeResponse)
}
...
func doFIP0115BaseFeeResponse(currentHeight abi.ChainEpoch, b upgradeBoundary) {
	...
	case fip0115PhaseIdle:
		// Trigger window: from (epoch - preLead) through (epoch - 1).
	...
	case fip0115PhaseFlooded:
		// Sample peak after the flood has had a few epochs to digest.
	...
	case fip0115PhaseDuring:
		// Sample post-congestion recovery well after the flood.
...
// doBaseFeeAgreement covers cross-node consistency separately.
...
func runFIP0115Flood(msgCount, durationSec int, premiumAtto int64) (int, int) {
	...
	// GasFeeCap high enough to cover premium + any plausible basefee spike.
	...
	log.Printf("[fip0115] flood done: submitted=%d attempted=%d in %.1fs",
		submitted, attempted, time.Since(deadline.Add(-time.Duration(durationSec)*time.Second)).Seconds())
