...
def init_wallets(node_type):
    ...
    genesis_wallet_pk = wallets.get_wallets_private_keys(node_type="lotus", rpc_url=lotus_rpc_url, auth_token=lotus_auth_token, wallets=[genesis_wallet])[0]
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Error: initialize_wallets.py needs a node_type parameter (options: forest, lotus)")
    else:    
