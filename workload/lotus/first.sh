...
do
    echo "Workload [Lotus][first.sh]: Waiting for RPC endpoint (ChainHead at ${RPC_LOTUS}) to come online..."
...
echo "Workload [Lotus][first.sh]: ChainHead RPC endpoint online"

# Waiting for the chain head to pass a certain height
BLOCK_HEIGHT_REACHED=0
while [ $INIT_BLOCK_HEIGHT -gt $BLOCK_HEIGHT_REACHED ]
do
    echo "Workload [Lotus][first.sh]: Waiting for block to reach ${INIT_BLOCK_HEIGHT}"
    ...
    echo "Workload [Lotus][first.sh]: Block height check: reached ${BLOCK_HEIGHT_REACHED}"
    sleep 5
done
echo "Workload [Lotus][first.sh]: Chain head has reached target block height"

echo "Workload [Lotus][first.sh]: Initializing wallets..."
python3 -u /opt/antithesis/resources/initialize_wallets.py "lotus"

echo "Workload [Lotus][first.sh]: Completed workload setup."
